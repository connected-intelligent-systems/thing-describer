 ...
 * @type {string}
 */
const ThingsboardHttpDeviceApiUrl = env.get('THINGSBOARD_HTTP_DEVICE_API_URL').default("http://thingsboard:9090/api/v1").asString()
...
async function sendSyncStatus(token, status, message) {
  const response = await fetch(`${ThingsboardHttpDeviceApiUrl}/${token}/attributes`, {
    method: 'POST',
    body: JSON.stringify({
